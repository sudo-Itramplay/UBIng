...
// src/todoistApiClient.ts
var import_obsidian = require("obsidian");
var TODOIST_API_BASE_URL = "https://api.todoist.com/api/v1/";
var TODOIST_TASK_LINK_BASE_URL = "https://app.todoist.com/app/task/";
var MAX_FILTER_PAGES = 50;
function isTodoistObject(value) {
  return typeof value === "object" && value !== null;
}
function getTodoistRequestStatus(error) {
...
function getTodoistRequestData(error) {
  if (!isTodoistObject(error)) {
    return void 0;
  }
  if (error.json !== void 0) {
    return error.json;
  }
  if (error.text !== void 0) {
    return error.text;
  }
  ...
  if (error.response.json !== void 0) {
    return error.response.json;
  }
  return void 0;
}
function toTodoistRequestError(error) {
  if (error instanceof Error) {
    const existingError = error;
    if (existingError.httpStatusCode !== void 0 || existingError.responseData !== void 0) {
      return existingError;
    }
  }
  const status = getTodoistRequestStatus(error);
  const requestError = new Error(
    error instanceof Error ? error.message : `Request failed${status !== void 0 ? `, status ${status}` : ""}`
  );
  requestError.httpStatusCode = status;
  requestError.responseData = getTodoistRequestData(error);
  return requestError;
}
function buildTodoistUrl(path, queryParams) {
...
function todoistRequest(authToken, method, path, queryParams, body) {
  return __async(this, null, function* () {
    ...
    try {
      const response = yield (0, import_obsidian.requestUrl)(requestOptions);
      let responseData;
      try {
...
function normalizeTodoistTask(taskValue) {
  if (!isTodoistObject(taskValue) || taskValue.id === void 0) {
    return null;
  }
  ...
  const parentIdValue = taskValue.parentId !== void 0 ? taskValue.parentId : taskValue.parent_id;
  const parentId = parentIdValue === null || parentIdValue === void 0 || String(parentIdValue).length === 0 ? null : String(parentIdValue);
  const priority = typeof taskValue.priority === "number" ? taskValue.priority : 1;
  const taskUrl = typeof taskValue.url === "string" && taskValue.url.length > 0 ? taskValue.url : `${TODOIST_TASK_LINK_BASE_URL}${id}`;
  return {
    id,
...
function getTodoistTaskItems(responseData) {
...
function getNextCursor(responseData) {
  ...
  const nextCursor = typeof responseData.nextCursor === "string" ? responseData.nextCursor : typeof responseData.next_cursor === "string" ? responseData.next_cursor : void 0;
  return nextCursor && nextCursor.length > 0 ? nextCursor : void 0;
}
function getTodoistTasks(authToken, filter) {
  return __async(this, null, function* () {
    let tasks = [];
    let cursor;
    for (let page = 0; page < MAX_FILTER_PAGES; page++) {
      ...
      const response = yield todoistRequest(authToken, "GET", "tasks/filter", queryParams);
      const pageTasks = getTodoistTaskItems(response.data).map((task) => normalizeTodoistTask(task)).filter((task) => task !== null);
...
function getTodoistTask(authToken, taskId) {
  return __async(this, null, function* () {
    const response = yield todoistRequest(authToken, "GET", `tasks/${taskId}`);
...
function closeTodoistTask(authToken, taskId) {
  return __async(this, null, function* () {
    yield todoistRequest(authToken, "POST", `tasks/${taskId}/close`);
  });
}
function reopenTodoistTask(authToken, taskId) {
  return __async(this, null, function* () {
    yield todoistRequest(authToken, "POST", `tasks/${taskId}/reopen`);
  });
}
    ...
    for (const keywordToQuery of settings.keywordToTodoistQuery) {
      if (keywordToQuery.keyword.length > 1 && keywordToQuery.todoistQuery.length > 1 && fileContents.contains(keywordToQuery.keyword)) {
        if (settings.authToken.contains("TODO - ")) {
          new import_obsidian2.Notice("Todoist Text: You need to configure your Todoist API token in the Todoist Text plugin settings");
...
function toggleServerTaskStatus(e, settings) {
  return __async(this, null, function* () {
    try {
      ...
      const serverTaskName = (yield getTodoistTask(settings.authToken, taskId)).content;
      if (tryingToClose) {
        yield closeTodoistTask(settings.authToken, taskId);
        ...
        new import_obsidian2.Notice(taskClosedMessage);
      }
      if (tryingToReOpen) {
        yield reopenTodoistTask(settings.authToken, taskId);
        const actionedTaskTabCount = lineText.split(/[^\t]/)[0].length;
        ...
        new import_obsidian2.Notice(taskOpenedMessage);
      }
    } catch (e2) {
      console.log("todoist text error: ", e2);
      new import_obsidian2.Notice("Todoist Text: Error trying to update task status. See console log for more details.");
    }
  });
}
function getServerData(todoistQuery, authToken, showSubtasks) {
  return __async(this, null, function* () {
    const tasks = yield callTasksApi(todoistQuery, authToken);
    if (tasks.length === 0) {
      new import_obsidian2.Notice(`Todoist text: You have no tasks matching filter "${todoistQuery}"`);
    }
    let returnString = "";
    if (showSubtasks) {
...
function callTasksApi(filter, authToken) {
  return __async(this, null, function* () {
    let tasks;
    try {
      tasks = yield getTodoistTasks(authToken, filter);
    } catch (e) {
      ...
      switch (todoistError.httpStatusCode) {
        ...
        default: {
          let responseData = "";
          if (todoistError.responseData !== void 0) {
            try {
          ...
          errorMsg = responseData.length > 0 ? `Todoist text: There was a problem pulling data from Todoist. ${responseData}` : "Todoist text: There was a problem pulling data from Todoist.";
        }
      }
...
// main.ts
var TodoistPlugin = class extends import_obsidian5.Plugin {
  constructor() {
    super(...arguments);
...
var TodoistPluginSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  addEnableAutomaticReplacementSetting(containerEl) {
    new import_obsidian5.Setting(containerEl).setName("Enable automatic replacement of keyword with Todos").setDesc("When enabled, any time a keyword is seen in a non-excluded file, it will be automatically replaced with your Todos whenever the file is opened. When disabled, manually use the 'Replace keyword with todos' command to replace your keyword with Todos.").addToggle((t) => t.setValue(this.plugin.settings.enableAutomaticReplacement).onChange(
  ...
  addIncludeSubttasksSetting(containerEl) {
    new import_obsidian5.Setting(containerEl).setName("Enable Subtasks").setDesc("When enabled, any Subtasks associated with Todos meeting filter criteria from your keyword will be shown, indented, under the parent Todo.").addToggle((t) => t.setValue(this.plugin.settings.showSubtasks).onChange(
      (value) => __async(this, null, function* () {
  ...
  addExcludedDirectoriesSetting(containerEl) {
    ...
    new import_obsidian5.Setting(this.containerEl).setDesc(excludedFolderDescription);
    this.plugin.settings.excludedDirectories.forEach(
      (dir, index) => {
        new import_obsidian5.Setting(this.containerEl).setName("Excluded folder").addSearch((cb) => {
    ...
    new import_obsidian5.Setting(this.containerEl).setName("Add another excluded folder").addButton((button) => {
  ...
  addKeywordTodoistQuerySetting(containerEl) {
    ...
    new import_obsidian5.Setting(this.containerEl).setName("Add another keyword and Todoist query").addButton((button) => {
      button.setButtonText("+").setCta().onClick(() => __async(this, null, function* () {
  ...
  addApiKeySetting(containerEl) {
    ...
    new import_obsidian5.Setting(containerEl).setName("API token").setDesc(tokenDescription).addText((text) => text.setValue(this.plugin.settings.authToken).onChange((value) => __async(this, null, function* () {
